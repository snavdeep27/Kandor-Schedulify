...
import os, re, uuid, time, calendar, datetime as dt
...
.link-card input{
  ...
  font-family: ui-monospace, Menlo, monospace; font-size:14px; color:#1f2544;
}
.link-card button{
  padding:10px 16px; border-radius:999px; border:none; color:#fff;
...
def graph_create_event(token: str, subject: str, html_body: str,
    ...
    return (r.status_code in (201, 200), (r.json() if r.status_code in (201,200) else {"status": r.status_code, "text": r.text}))
...
def booking_page():
    ...
    with right:
        ...
        for i, t in enumerate(slots):
            with cols[i % 3]:
                if st.button(pretty(t), key=f"slot_{t}"):
    ...
    st.markdown(
        ...
        <script>
          (function(){{
            ...
            }} catch (e) {{ console.warn("Auto-redirect suppressed:", e); }}
          }})();
        </script>
        """,
        unsafe_allow_html=True,
